...
def on_generate_button_clicked():
  ...
  def set_processed_midi():
    line_edit_processed_midi.setText(QFileDialog.getOpenFileName (None, 'Select processed midi file (melodies)')[0])
  ...
  buttonBox.accepted.connect(lambda: accept_and_finish([
    line_edit_processed_midi.text(),
    line_edit_weights.text(),
    line_edit_output_midi_file.text(),
    str(combo_box_pitch.currentData()) + str(combo_box_accidental.currentData()) + str(spin_box_scale.value()), 
    combo_box_instruments.currentData(),
    combo_box_scale.currentData(),
    spin_box_notes.value
  ], generate, dialog))
  buttonBox.rejected.connect(dialog.reject)
  layout.addWidget(buttonBox)
